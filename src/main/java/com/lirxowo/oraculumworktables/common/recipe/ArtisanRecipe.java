...
public abstract class ArtisanRecipe
  ...
  public boolean matches(ArtisanInventory inventory, Level world) {
    ...
    if (!this.fluidIngredient.isEmpty()) {
      ...
      if (!FluidStack.isSameFluidSameComponents(tankFluid, this.fluidIngredient)
          || tankFluid.getAmount() < this.fluidIngredient.getAmount()) {
        return false;
      }
    }
  ...
  public boolean hasSufficientToolDurability(IToolHandler handler, ItemStack tool) {
    ...
    if (OraculumWorktablesModCommonConfig.restrictCraftMinimumDurability) {
